...
function whoWon() {
    ...
    if (gameResult === "rock beats scissors" && computerInput === 0) {
        ...
        return "you won! " + gameResult
    } else if (gameResult === "invalid input"){
        return "invalid input " + gameResult
    } else {
...
function playRound(x,y){
    let roundResult = whoWon();
    return roundResult
}

//these arrays hold score for each player`
let computerWins = [];
let playerWins = [];

function game() {
    for (let i = 0; i < 5; i++) {
        playerInput; //it wont loop without declaring a variable
        computerInput;
